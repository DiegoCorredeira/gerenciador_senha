...
def add_new_pass():
    ...
    username_entry = tk.Entry(add_pass_window, textvariable=username_var)
    username_entry.pack()
    
    tk.Label(add_pass_window, text="Senha").pack()
    password_entry = tk.Entry(add_pass_window, show='*', textvariable=password_var)
    password_entry.pack()
...
def search_in_db():
    ...
    tk.Label(search_pass_db,text="Informe o nome do site que deseja buscar a senha").pack()
    ...
    def show_password():
        ...
        if result:
            ...
            output_label.config(text=f"Site: {result['Site']}\nUsername: {result['Username']}\nPassword: {result['password']}")
        else:
...
dropped_menu = tk.Menu(menubar, tearoff=0)
# dropped_menu.configure(tearoffcommand=dropped_menu.winfo_width)
menubar.add_cascade(label="O que deseja fazer?", menu=dropped_menu)
dropped_menu.add_command(label="Adicionar nova senha", command=add_new_pass)
dropped_menu.add_command(label="Buscar senha", command=search_in_db)
dropped_menu.add_command(label="Atualizar senha", command=update_password_gui)
dropped_menu.add_command(label="Remover senha")

dropped_menu.add_separator()
dropped_menu.add_command(label="Sair", command=root.quit)
